// ============================================================
// staff/tabs/Home.js — DEMO VERSION (no Firebase)
// Firebase listeners replaced with localStorage + props data
// ============================================================
...
// ── Demo: use finePerms from session (no live Firebase sync) ──
function useFirebasePerms(sess) {
  return sess?.finePerms || defaultFinePerms(sess?.role || "factory");
}

// ── Demo: notices come from settings.noticeBoard ──────────────
function useFirebaseNotices(settings) {
  return Array.isArray(settings?.noticeBoard) ? settings.noticeBoard : [];
}

// ── Demo: wastage comes from props passed down ─────────────────
function useFirebaseWastage(wastage) {
  const today = new Date().toISOString().slice(0, 10);
  return Array.isArray(wastage) ? wastage.filter(w => (w.date || "").startsWith(today)) : [];
}
                ...
                return (
                  <div key={b.id} style={{ display: "flex", alignItems: "center", gap: 9 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ height: 4, background: "rgba(255,255,255,0.06)", borderRadius: "999px", overflow: "hidden" }}>
                        <div style={{ height: "100%", width: `${maxV > 0 ? ((b.actual ?? 0) / maxV) * 100 : 0}%`, background: c, borderRadius: "999px", transition: "width 0.5s ease" }} />
                      </div>
                    </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: bottomCols, gap: 14 }}>
        <div>
          <SSectionHeader title="Recent Activity" t={t} accent={t.cyan}
            action={<span onClick={() => setActiveTab("reports")} style={{ color: t.cyan, fontSize: 10, fontWeight: 700, cursor: "pointer" }}>View all →</span>}
          />
        ...
        <div>
          <SSectionHeader title="Active Batches" t={t} accent={TAB_ACCENT.packing.solid}
            action={<span onClick={() => setActiveTab("packing")} style={{ color: TAB_ACCENT.home?.solid || "#6366f1", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>View all →</span>}
          />
          <div style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, overflow: "hidden", backdropFilter: "blur(20px)", boxShadow: "0 4px 24px rgba(0,0,0,0.25)", marginTop: 10 }}>
    ...
    <div style={{ background: t.bg, minHeight: "100vh", padding: isMobile ? "14px 12px 28px" : "18px 18px 32px", animation: "fadeIn 0.3s ease" }}>
      ...
      {overdue.length > 0 && (
        <div style={{
          ...
          <button onClick={() => setActiveTab("delivery")} style={{ padding: "7px 14px", borderRadius: 9, border: "none", background: "rgba(239,68,68,0.15)", color: "#ef4444", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>View →</button>
        </div>
      )}
      ...
      <div>
        <SSectionHeader title="Up Next" t={t} accent={TAB_ACCENT.delivery.solid}
          action={<span onClick={() => setActiveTab("delivery")} style={{ color: TAB_ACCENT.delivery.solid, fontSize: 10, fontWeight: 700, cursor: "pointer" }}>View all →</span>}
        />
        <div style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, overflow: "hidden", backdropFilter: "blur(20px)", boxShadow: "0 4px 24px rgba(0,0,0,0.25)", marginTop: 10 }}>
          {upNext.length === 0 ? (
            ...
            return (
              <div key={d.id}
                ...
                style={{ padding: "14px 16px", borderBottom: i < arr.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none", borderLeft: `3px solid ${sc}`, cursor: "pointer", transition: "all 0.15s", background: isOverdue ? "rgba(239,68,68,0.04)" : "transparent" }}
                onMouseEnter={e => { e.currentTarget.style.background = "rgba(255,255,255,0.035)"; }}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 3, flexWrap: "wrap" }}>
                      ...
                      <span style={{ background: `${sc}15`, color: sc, border: `1px solid ${sc}30`, borderRadius: 6, padding: "2px 7px", fontSize: 9, fontWeight: 700 }}>
                        {STATUS_ICON[d.status]} {d.status}
                      </span>
                      {isOverdue && <span style={{ background: "rgba(239,68,68,0.15)", color: "#ef4444", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 6, padding: "2px 7px", fontSize: 9, fontWeight: 700 }}>OVERDUE</span>}
                    </div>
...
// ═══════════════════════════════════════════════════════════════
export function HomeTab({ t, sess, batches = [], inventory = [], deliveries = [], staffList = [], qcLogs = [], wastage = [], setActiveTab, notify, settings }) {
  ...
  const notices      = useFirebaseNotices(settings);
  const todayWastage = useFirebaseWastage(wastage);
